...
test('Should update valid user fields', async () => {
  const response = await request(app)
    ...
    .send({
      ...
      email: 'kimbee.mancuyas@gmail.com',
      password: 'kimbeepassword'
    })
    .expect(200);

  const user = await User.findById(userOneId);
  expect(user.name).toEqual(response.body.user.name);
  expect(user.email).toEqual(response.body.user.email);
  expect(user.password).toEqual(response.body.user.password);
});
